...
export const useTranslateStore = defineStore('translate', () => {
  ...
  async function loadReferenceData() {
    try {
      const [langs, pos] = await Promise.all([
        api.fetchLanguages(),
        api.fetchPartsOfSpeech(),
      ])
  ...
  async function checkDatabaseStatus() {
    ...
    try {
      if (hasRoot) {
        ...
        inputWordStatus.value = findExactMatch(inputResults.words, result.value.source_text) ?? false
        rootWordStatus.value = findExactMatch(rootResults.words, result.value.root_source) ?? false
      } else {
  ...
  async function doSaveInput() {
    ...
    try {
      const saved = await api.saveWord({
        translations: [
  ...
  return {
    ...
    inputText, context, partOfSpeechInput, partOfSpeechRoot,
    result, inputWordStatus, rootWordStatus,
    serviceAvailable, loading, savingInput, savingRoot, error,
    languages, partsOfSpeech,
    checkHealth, loadReferenceData, swapLanguages,
    doTranslate, doSaveInput, doSaveRoot,
  }
}, {
  persist: {
