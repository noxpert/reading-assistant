...
- **Node.js 20+** — only needed for local development (`make dev`)
- **Translation service** — must be running on port 8081 before using the app

## Node.js Installation (for `make dev` only)
...
VITE_SOURCE_NAME=vocab-app               # tag applied to saved words
```
...
│   │   ├── WordPanel.vue     # Word/root word panel with save
